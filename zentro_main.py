...
def get_exchange_rate():
    rate = safe_float(get_setting("usd_to_syp", "0"), 0.0)
    # 1 or less means the Syrian-pound exchange rate has not been configured yet.
...
async def admin_money(query):
    ...
    rate = get_exchange_rate()
    rate_text = f"1$ = {rate:,.0f} ل.س" if rate else "غير محدد"
    lines = [
        "💵 <b>الرصيد والأسعار</b>\n",
        f"\n💱 سعر الصرف الحالي: <b>{rate_text}</b>\n",
    ]
    for p in providers:
        lines.append(
            f"\n🔗 {html.escape(p['name'])}\n"
    ...
    await query.edit_message_text(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [blue("💱 تعديل سعر الصرف", "setting_exchange_rate")],
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "home":
        ...
        if not rate:
            await query.answer(
                "⚠️ سعر الصرف غير مضبوط بعد. تواصل مع الإدارة.",
                show_alert=True,
            )
            return
        await query.edit_message_text(
    ...
    # ADMIN
        ...
        if data == "admin_stats":
        ...
        elif data == "setting_exchange_rate":
            context.user_data.clear()
            ...
            await query.edit_message_text(
                ...
                "أرسل سعر 1 دولار بالليرة السورية.\n"
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "manual_deposit_amount":
        ...
        if not rate:
            context.user_data.clear()
            await update.message.reply_text("⚠️ سعر الصرف غير مضبوط بعد. تواصل مع الإدارة.")
            return
