...
class SMMProvider:
    ...
    async def balance(self):
        ...
        if not isinstance(data, dict):
            raise RuntimeError("رد الرصيد غير متوقع.")
        return {
    ...
    async def services(self):
        ...
        if not isinstance(data, list):
            raise RuntimeError("المصدر لم يرجع قائمة خدمات.")
    ...
    async def add_order(self, service_id, target, quantity):
        ...
        if order_id is None:
            raise RuntimeError(f"المصدر لم يرجع رقم طلب: {str(data)[:250]}")
...
async def test_provider(provider_id):
    ...
    if not provider:
        raise RuntimeError("المصدر غير موجود.")
...
def admin_menu():
    return InlineKeyboardMarkup(
        [
            [blue("📊 الإحصائيات", "admin_stats"), red("🔗 المصادر API", "providers:0")],
    ...
    try:
        await app.bot.set_my_description(
            description=(
                ...
                "متجر خدمات رقمية أوتوماتيكي.\n"
                "🔗 ربط مصادر API\n"
                "⚡ تنفيذ ومتابعة الطلبات تلقائياً"
            )
        )
        await app.bot.set_my_short_description(
            short_description="متجر رقمي أوتوماتيكي • API • طلبات • خدمات"
        )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "🤖 متجر خدمات رقمي أوتوماتيكي\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "🔗 يسحب الخدمات والأسعار من المصدر\n"
        "⚡ يرسل الطلب للمصدر تلقائياً\n"
        "🔄 يتابع حالة الطلب تلقائياً\n"
        "💸 يرجع الرصيد تلقائياً عند إلغاء المصدر\n"
        "━━━━━━━━━━━━━━━━━━\n\n"
        "اختر من القائمة 👇",
        parse_mode="HTML",
...
async def show_store(query, page=0):
    ...
    await query.edit_message_text(
        "🛍️ <b>أقسام المتجر</b>\n\nالأقسام تُسحب تلقائياً من المصدر.",
        parse_mode="HTML",
...
async def show_order(query, order_id):
    ...
    await query.edit_message_text(
        ...
        f"🔗 رقم المصدر: <code>{html.escape(order['provider_order_id'] or '-')}</code>\n"
...
async def show_about(query):
    ...
    await query.edit_message_text(
        ...
        "🤖 متجر أوتوماتيكي\n"
        f"🔗 مصادر فعالة: <b>{providers}</b>\n"
        f"🛍️ خدمات فعالة: <b>{products}</b>\n"
        "🔄 مزامنة الأسعار وحالات الطلبات تلقائياً",
        parse_mode="HTML",
        reply_markup=back_home(),
    )
...
async def show_deposit(query, context):
    if not TELEGRAM_PAYMENT_TOKEN:
        await query.edit_message_text(
            "💳 <b>الشحن الأوتوماتيكي</b>\n\n"
            "ما في بوابة دفع أوتوماتيكية مضافة حالياً.\n\n"
            "للدفع عبر Telegram Payments أضف "
            "<code>TELEGRAM_PAYMENT_TOKEN</code> في Railway.\n\n"
            "أما شام كاش أو سيريتل كاش أو أي بوابة ثانية، فلازم يكون عندها API/Webhook رسمي.",
            parse_mode="HTML",
            reply_markup=back_home(),
        )
        return
...
async def admin_stats(query):
    ...
    await query.edit_message_text(
        ...
        f"🔗 المصادر الفعالة: <b>{providers}</b>\n"
...
async def providers_page(query, page=0):
    ...
    rows = [[blue("➕ ربط مصدر جديد", "provider_add")]]
    ...
    await query.edit_message_text(
        "🔗 <b>مصادر الخدمات API</b>\n\n"
        "يدعم تلقائياً Standard SMM API v2: services / balance / add / status.\n"
        "إذا مصدر معين يستخدم API مختلف، يحتاج Adapter خاص له فقط.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(rows),
    )
...
async def provider_page(query, provider_id):
    ...
    await query.edit_message_text(
        ...
        f"💰 رصيد المصدر: <b>{provider['last_balance'] if provider['last_balance'] is not None else '-'} {html.escape(provider['last_currency'] or '')}</b>\n"
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                ...
                [
                    ...
                    red("🗑 حذف المصدر", f"provider_delete:{provider_id}"),
                ],
                [red("⬅️ المصادر", "providers:0")],
            ]
        ),
    )
...
async def admin_products(query, page=0):
    ...
    if not rows:
        rows.append([blue("📭 اسحب الخدمات من المصدر", "providers:0")])
...
async def admin_product_page(query, product_id):
    ...
    await query.edit_message_text(
        f"🛍️ <b>{html.escape(product['name'])}</b>\n\n"
        f"🔗 المصدر: {html.escape(product['provider_name'])}\n"
...
async def admin_order_page(query, order_id):
    ...
    await query.edit_message_text(
        ...
        f"🛍️ {html.escape(order['product_name'])}\n"
        f"🔗 {html.escape(order['provider_name'])}\n"
        f"🧾 Source: <code>{html.escape(order['provider_order_id'] or '-')}</code>\n"
        f"🔢 الكمية: <b>{order['quantity']}</b>\n"
...
async def admin_settings(query):
    await query.edit_message_text(
        ...
        f"💳 الدفع الأوتوماتيكي: {'فعال ✅' if TELEGRAM_PAYMENT_TOKEN else 'غير مضاف ⛔'}",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(
            [
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ADMIN
        ...
        if data == "admin_stats":
            ...
            await query.edit_message_text("🔄 جاري سحب الخدمات والأسعار...")
...
async def confirm_buy(query, context):
    ...
    try:
        ...
        await query.edit_message_text(
            "❌ <b>المصدر رفض الطلب</b>\n\n"
            "رجعنا رصيدك تلقائياً.\n\n"
    ...
    await query.edit_message_text(
        ...
        f"🔗 رقم المصدر: <code>{html.escape(provider_order_id)}</code>\n"
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "provider_name":
        ...
        await update.message.reply_text(
            "🌐 أرسل رابط API الكامل كما يعطيك المصدر.\n"
            "مثال: <code>https://provider.com/api/v2</code>\n\n"
            "لا تخمّن الرابط؛ خذه من صفحة API عند المصدر.",
            parse_mode="HTML",
        )
        return
    ...
    if state == "provider_profit_new":
        ...
        await update.message.reply_text(
            f"✅ تم حفظ المصدر <b>{html.escape(name)}</b>.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(
                [
                    [blue("🧪 فحص الاتصال", f"provider_test:{provider_id}")],
                    [red("🔗 فتح المصدر", f"provider:{provider_id}")],
                ]
            ),
        )
        return
    ...
    if state == "provider_profit":
        ...
        await update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[blue("🔗 المصدر", f"provider:{provider_id}")]]),
        )
        return
    ...
    if refund > 0:
        try:
            await app.bot.send_message(
                ...
                f"لأن حالة المصدر صارت: <b>{html.escape(raw_status)}</b>",
                parse_mode="HTML",
            )
        except TelegramError:
            pass
