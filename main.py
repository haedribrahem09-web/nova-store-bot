    ...
    with db() as conn:
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS deposits (
                ...
                created_at TEXT NOT NULL,
                updated_at TEXT NOT NULL,
                FOREIGN KEY(method_code) REFERENCES deposit_methods(code)
            );
            """
        )

        # Safe migration for stores created with an older version.
        deposit_columns = {
            row[1] for row in conn.execute("PRAGMA table_info(deposit_methods)").fetchall()
        }
        if "address" not in deposit_columns:
            conn.execute(
                "ALTER TABLE deposit_methods ADD COLUMN address TEXT NOT NULL DEFAULT ''"
            )

        deposit_request_columns = {
            row[1] for row in conn.execute("PRAGMA table_info(deposits)").fetchall()
        }
        if "local_amount" not in deposit_request_columns:
            conn.execute("ALTER TABLE deposits ADD COLUMN local_amount REAL")
        if "exchange_rate" not in deposit_request_columns:
            conn.execute("ALTER TABLE deposits ADD COLUMN exchange_rate REAL")
        conn.execute("UPDATE deposits SET local_amount = amount WHERE local_amount IS NULL")

        defaults = {
            "store_name": STORE_NAME_ENV,
            "support_username": SUPPORT_USERNAME_ENV,
            "usd_to_syp": os.getenv("USD_TO_SYP", "0"),
            "sham_cash_to_syp": os.getenv("SHAM_CASH_TO_SYP", "0"),
            "syriatel_cash_to_syp": os.getenv("SYRIATEL_CASH_TO_SYP", "0"),
        }
        ...
        # سعر الدولار ثابت دائماً 1:1 ولا نستخدمه كسعر ليرة.
        conn.execute(
            "INSERT INTO settings(key, value) VALUES('usd_to_syp', '1') "
            "ON CONFLICT(key) DO UPDATE SET value = '1'"
        )

        # Migration from the old single exchange-rate setting: preserve the old rate
        # as the initial Sham Cash / Syriatel Cash rate until you edit each separately.
        old_rate_row = conn.execute(
            "SELECT value FROM settings WHERE key = 'usd_to_syp'"
        ).fetchone()
        try:
            old_rate = float(old_rate_row[0]) if old_rate_row else 0.0
        except Exception:
            old_rate = 0.0
        if old_rate > 1:
            for rate_key in ("sham_cash_to_syp", "syriatel_cash_to_syp"):
                current = conn.execute(
                    "SELECT value FROM settings WHERE key = ?",
                    (rate_key,),
                ).fetchone()
                try:
                    current_rate = float(current[0]) if current else 0.0
                except Exception:
                    current_rate = 0.0
                if current_rate <= 1:
                    conn.execute(
                        ...
                        (rate_key, str(old_rate)),
                    )

        now = now_iso()
        conn.execute(
            """
            INSERT OR IGNORE INTO deposit_methods(code, name, address, description, active, updated_at)
            VALUES('USD', 'دولار', '', 'حوّل المبلغ بالدولار إلى العنوان الظاهر بالأسفل، ثم أرسل صورة إثبات التحويل.', 1, ?)
            """,
            (now,),
        )
        conn.execute(
            """
            INSERT OR IGNORE INTO deposit_methods(code, name, address, description, active, updated_at)
            VALUES('SHAM', 'شام كاش', '', 'حوّل المبلغ إلى عنوان شام كاش الظاهر بالأسفل، ثم أرسل صورة إثبات التحويل.', 1, ?)
            """,
            (now,),
        )
        conn.execute(
            """
            INSERT OR IGNORE INTO deposit_methods(code, name, address, description, active, updated_at)
            VALUES('SYRIATEL', 'سيريتل كاش', '', 'حوّل المبلغ إلى عنوان سيريتل كاش الظاهر بالأسفل، ثم أرسل صورة إثبات التحويل.', 1, ?)
            """,
            (now,),
        )
...
def get_exchange_rate(rate_code="USD"):
    rate_code = (rate_code or "USD").upper()

    # الدولار دولار: 1 USD يدخل = 1 USD رصيد، ثابت وغير قابل للتعديل.
    if rate_code == "USD":
        return 1.0

    setting_key = {
        "SHAM": "sham_cash_to_syp",
        "SYRIATEL": "syriatel_cash_to_syp",
    }.get(rate_code)

    if not setting_key:
        return 0.0

    rate = safe_float(get_setting(setting_key, "0"), 0.0)
    return rate if rate > 1 else 0.0


def exchange_rate_name(rate_code):
    return {
        "USD": "الدولار",
        "SHAM": "شام كاش",
        "SYRIATEL": "سيريتل كاش",
    }.get((rate_code or "USD").upper(), "الدولار")


def exchange_rate_setting_key(rate_code):
    return {
        "USD": "usd_to_syp",
        "SHAM": "sham_cash_to_syp",
        "SYRIATEL": "syriatel_cash_to_syp",
    }.get((rate_code or "USD").upper(), "usd_to_syp")
...
def admin_menu():
    return InlineKeyboardMarkup(
        [
            ...
            [blue("💳 طرق الدفع", "admin_payment_methods"), red("📥 طلبات الإيداع", "admin_deposits:0")],
...
async def show_deposit(query, context):
    ...
    with db() as conn:
        methods = conn.execute(
            "SELECT * FROM deposit_methods WHERE active = 1 ORDER BY name"
        ).fetchall()

    rows = []
    for method in methods:
        rows.append([
            blue(f"💳 {method['name']}", f"dep_method:{method['code']}")
        ])

    if not rows:
        rows.append([blue("📭 لا توجد طرق دفع متاحة حالياً", "noop")])

    rows.append([red("⬅️ العودة للرئيسية", "home")])
...
async def show_deposit_method(query, context, method_code):
    with db() as conn:
        method = conn.execute(
            "SELECT * FROM deposit_methods WHERE code = ? AND active = 1",
            (method_code,),
        ).fetchone()

    if not method:
        await query.answer("طريقة الدفع غير متاحة حالياً.", show_alert=True)
        return

    context.user_data.clear()
    context.user_data["deposit_method_code"] = method_code

    address = (method['address'] or '').strip()
    address_text = (
        f"📍 <b>عنوان التحويل:</b>\n<code>{html.escape(address)}</code>"
        if address
        else "⚠️ <b>عنوان التحويل غير محدد حالياً.</b>"
    )

    rate = get_exchange_rate(method_code)
    if method_code == "USD":
        rate_text = "1$ = 1$ (ثابت)"
        amount_hint = "بعد التحويل اضغط متابعة، ثم أرسل المبلغ الذي حولته <b>بالدولار</b> وصورة الإثبات."
    else:
        rate_text = f"1$ = {rate:,.0f} ل.س" if rate else "غير محدد بعد"
        amount_hint = "بعد التحويل اضغط متابعة، ثم أرسل المبلغ الذي حولته <b>بالليرة السورية</b> وصورة الإثبات."

    await query.edit_message_text(
        f"💳 <b>{html.escape(method['name'])}</b>\n\n"
        f"{address_text}\n\n"
        f"📝 {html.escape(method['description'])}\n\n"
        f"💱 <b>سعر الصرف:</b> {rate_text}\n\n"
        f"{amount_hint}",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [blue("✅ متابعة الإيداع", f"dep_continue:{method_code}")],
            [red("⬅️ طرق الدفع", "deposit")],
        ]),
    )
...
async def admin_money(query):
    ...
    sham_rate = get_exchange_rate("SHAM")
    syriatel_rate = get_exchange_rate("SYRIATEL")

    def fmt(rate):
        return f"1$ = {rate:,.0f} ل.س" if rate else "غير محدد"

    lines = [
        "💵 <b>الرصيد والأسعار</b>\n",
        "\n💵 الدولار: <b>1$ = 1$ (ثابت)</b>",
        f"\n💳 شام كاش: <b>{fmt(sham_rate)}</b>",
        f"\n📱 سيريتل كاش: <b>{fmt(syriatel_rate)}</b>\n",
    ]
    ...
    if not providers:
        lines.append("\nلا يوجد API مربوط حالياً.")

    await query.edit_message_text(
        "".join(lines),
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [blue("💳 تعديل سعر شام كاش", "setting_exchange_rate:SHAM")],
            [blue("📱 تعديل سعر سيريتل كاش", "setting_exchange_rate:SYRIATEL")],
...
async def admin_payment_methods(query):
    with db() as conn:
        methods = conn.execute(
            "SELECT * FROM deposit_methods ORDER BY name"
        ).fetchall()
    ...
    for method in methods:
        rows.append([
            blue(
                f"{'✅' if method['active'] else '⛔'} {method['name']}",
                f"admin_payment_method:{method['code']}",
            )
        ])
    ...
    await query.edit_message_text(
        "💳 <b>طرق الدفع</b>\n\n"
        "من هون بتحدد عنوان التحويل والوصف للدولار وشام كاش وسيريتل كاش، وبتفعل أو توقف كل طريقة.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(rows),
    )


async def admin_payment_method(query, method_code):
    with db() as conn:
        method = conn.execute(
            "SELECT * FROM deposit_methods WHERE code = ?",
            (method_code,),
        ).fetchone()
    ...
    current_address = (method['address'] or '').strip() or 'غير محدد'

    await query.edit_message_text(
        f"💳 <b>{html.escape(method['name'])}</b>\n\n"
        f"📌 الحالة: {'مفعلة ✅' if method['active'] else 'متوقفة ⛔'}\n\n"
        f"📍 <b>عنوان التحويل الحالي:</b>\n<code>{html.escape(current_address)}</code>\n\n"
        f"📝 <b>الوصف الحالي:</b>\n{html.escape(method['description'])}",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [blue("📍 تعديل عنوان التحويل", f"payment_address:{method_code}")],
            [blue("📝 تعديل الوصف", f"payment_desc:{method_code}")],
            [red("⏯ تفعيل / إيقاف", f"payment_toggle:{method_code}")],
            [red("⬅️ طرق الدفع", "admin_payment_methods")],
        ]),
    )


async def admin_deposits(query, page=0):
    per_page = 8
    offset = page * per_page
    ...
    with db() as conn:
        ...
        deposits = conn.execute(
            """
            SELECT d.*, m.name AS method_name, u.first_name
            FROM deposits d
            JOIN deposit_methods m ON m.code = d.method_code
    ...
    rows = []
    for dep in deposits:
        name = dep["first_name"] or str(dep["user_id"])
        shown_amount = dep["local_amount"] if dep["local_amount"] is not None else dep["amount"]
        amount_text = (
            f"{shown_amount:.2f}$"
            if dep["method_code"] == "USD"
            else f"{shown_amount:,.0f} ل.س"
        )
        rows.append([
            blue(
                f"📥 #{dep['id']} | {name[:14]} | {amount_text}",
                f"admin_deposit:{dep['id']}",
            )
        ])

    if not rows:
        rows.append([blue("✅ لا توجد إيداعات معلقة", "noop")])

    nav = nav_row("admin_deposits", page, total, per_page)
    if nav:
        rows.append(nav)
...
async def admin_deposit_show(query, context, deposit_id):
    with db() as conn:
        dep = conn.execute(
            """
            SELECT d.*, m.name AS method_name, u.first_name, u.username
            FROM deposits d
            JOIN deposit_methods m ON m.code = d.method_code
            LEFT JOIN users u ON u.user_id = d.user_id
            WHERE d.id = ?
    ...
    if not dep:
        await query.answer("طلب الإيداع غير موجود.", show_alert=True)
        return

    local_amount = dep["local_amount"] if dep["local_amount"] is not None else dep["amount"]
    rate = dep["exchange_rate"] if dep["exchange_rate"] else 1

    if dep["method_code"] == "USD":
        transfer_line = f"💵 المحوّل: <b>{local_amount:.2f}$</b>"
        rate_line = "💱 الصرف: <b>1$ = 1$</b>"
    else:
        transfer_line = f"💴 المحوّل: <b>{local_amount:,.0f} ل.س</b>"
        rate_line = f"💱 الصرف: <b>1$ = {rate:,.0f} ل.س</b>"

    caption = (
        f"📥 <b>إيداع #{dep['id']}</b>\n\n"
        f"👤 {html.escape(dep['first_name'] or '')}\n"
        f"🆔 <code>{dep['user_id']}</code>\n"
        f"💳 {html.escape(dep['method_name'])}\n"
        f"{transfer_line}\n"
        f"{rate_line}\n"
        f"💵 الرصيد المضاف: <b>{dep['amount']:.2f}$</b>\n"
        f"📌 {html.escape(dep['status'])}"
    )
    ...
    await context.bot.send_photo(
        ...
        reply_markup=InlineKeyboardMarkup([
            [
                blue("✅ قبول", f"deposit_approve:{deposit_id}"),
                red("❌ رفض", f"deposit_reject:{deposit_id}"),
            ]
        ]),
    )
...
async def admin_settings(query):
    await query.edit_message_text(
        ...
        "💳 طرق الدفع: <b>دولار • شام كاش • سيريتل كاش</b>",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(
            [
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    if not (data.startswith("deposit_approve:") or data.startswith("deposit_reject:")):
        await query.answer()
    ...
    if data == "home":
        ...
        await show_deposit_method(query, context, data.split(":", 1)[1])

    elif data.startswith("dep_continue:"):
        method_code = data.split(":", 1)[1]
        with db() as conn:
            method = conn.execute(
                "SELECT * FROM deposit_methods WHERE code = ? AND active = 1",
                (method_code,),
        ...
        if not method:
            ...
            return
        context.user_data.clear()
        context.user_data["state"] = "manual_deposit_amount"
        context.user_data["deposit_method_code"] = method_code
        rate = get_exchange_rate(method_code)
        if not rate:
            await query.answer(
                f"⚠️ سعر صرف {exchange_rate_name(method_code)} غير مضبوط بعد. تواصل مع الإدارة.",
                show_alert=True,
            )
            return

        if method_code == "USD":
            rate_line = "💱 السعر: <b>1$ = 1$</b>"
            amount_line = "أرسل المبلغ الذي حولته <b>بالدولار</b>.\nمثال: <code>10</code>"
        else:
            rate_line = f"💱 سعر الصرف: <b>1$ = {rate:,.0f} ل.س</b>"
            amount_line = "أرسل المبلغ الذي حولته <b>بالليرة السورية</b>.\nمثال: <code>100000</code>"
        ...
        await query.edit_message_text(
            ...
            f"{amount_line}",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[red("❌ إلغاء", "deposit")]]),
        )
    ...
    # ADMIN
    ...
    elif data.startswith("admin") or data.startswith("provider") or data.startswith("product_toggle") or data.startswith("category_") or data.startswith("order_refresh") or data.startswith("user_") or data.startswith("setting_") or data.startswith("payment_") or data.startswith("deposit_"):
        if not is_admin(query.from_user.id):
            await query.answer("❌ غير مصرح لك.", show_alert=True)
            return
        ...
        if data == "admin_stats":
            ...
            await admin_payment_method(query, data.split(":", 1)[1])

        elif data.startswith("payment_address:"):
            method_code = data.split(":", 1)[1]
            context.user_data.clear()
            context.user_data["state"] = "payment_address"
            context.user_data["payment_method_code"] = method_code
            await query.edit_message_text(
                "📍 أرسل عنوان التحويل لهذه الطريقة الآن.\n\n"
                "مثلاً رقم الحساب أو رقم الهاتف الذي سيحوّل إليه العميل.",
                reply_markup=InlineKeyboardMarkup([[red("❌ إلغاء", f"admin_payment_method:{method_code}")]]),
            )

        elif data.startswith("payment_desc:"):
            method_code = data.split(":", 1)[1]
            context.user_data.clear()
            context.user_data["state"] = "payment_desc"
            context.user_data["payment_method_code"] = method_code
            await query.edit_message_text(
                "📝 أرسل الوصف أو تعليمات التحويل الجديدة.",
                reply_markup=InlineKeyboardMarkup([[red("❌ إلغاء", f"admin_payment_method:{method_code}")]]),
            )
            ...
            with db() as conn:
                row = conn.execute(
                    "SELECT active FROM deposit_methods WHERE code = ?",
                    (method_code,),
                ).fetchone()
                if row:
                    conn.execute(
                        "UPDATE deposit_methods SET active = ?, updated_at = ? WHERE code = ?",
                        (0 if row["active"] else 1, now_iso(), method_code),
                    )
            await admin_payment_method(query, method_code)

        elif data.startswith("admin_deposits:"):
            await admin_deposits(query, int(data.split(":", 1)[1]))

        elif data.startswith("admin_deposit:"):
            await admin_deposit_show(query, context, int(data.split(":", 1)[1]))

        elif data.startswith("deposit_approve:"):
            ...
            with db() as conn:
                ...
                if not dep:
                    await query.answer("طلب الإيداع غير موجود.", show_alert=True)
                    return
                if dep["method_code"] != "USD" and (
                    dep["exchange_rate"] is None or float(dep["exchange_rate"] or 0) <= 1
                ):
                    await query.answer(
                        "⚠️ هذا طلب قديم قبل نظام سعر الصرف. ارفضه واطلب من العميل إنشاء إيداع جديد بعد ضبط سعر الصرف.",
                        show_alert=True,
                    )
                    return

                # Atomic update: the balance can only be credited once.
                cur = conn.execute(
                    "UPDATE deposits SET status = 'approved', updated_at = ? WHERE id = ? AND status = 'pending'",
                    (now_iso(), deposit_id),
                )
                if cur.rowcount != 1:
                ...
                conn.execute(
                    ...
                    (dep["amount"], now_iso(), dep["user_id"]),
                )

            try:
                await context.bot.send_message(
                    dep["user_id"],
                    f"✅ تم قبول إيداعك #{deposit_id} وإضافة <b>{dep['amount']:.2f}$</b> إلى رصيدك.",
                    parse_mode="HTML",
                )
            ...
            try:
                if query.message and query.message.photo:
                    old_caption = query.message.caption_html or query.message.caption or ""
                    await query.edit_message_caption(
                        caption=f"{old_caption}\n\n✅ <b>تم قبول الإيداع وإضافة الرصيد</b>",
                        parse_mode="HTML",
                        reply_markup=None,
                    )
                else:
                ...
                logger.warning("Could not update approved deposit message: %s", exc)

            await query.answer("✅ تم قبول الإيداع وإضافة الرصيد.", show_alert=True)
            ...
            with db() as conn:
                ...
                cur = conn.execute(
                    "UPDATE deposits SET status = 'rejected', updated_at = ? WHERE id = ? AND status = 'pending'",
                    (now_iso(), deposit_id),
                )
                if cur.rowcount != 1:
                    await query.answer("تمت معالجة هذا الطلب مسبقاً.", show_alert=True)
            ...
            try:
                await context.bot.send_message(
                    dep["user_id"],
                    f"❌ تم رفض طلب الإيداع #{deposit_id}. إذا في مشكلة تواصل مع الدعم الفني.",
                )
            except TelegramError:
                pass
            ...
            try:
                ...
                logger.warning("Could not update rejected deposit message: %s", exc)
        ...
        elif data == "setting_exchange_rate" or data.startswith("setting_exchange_rate:"):
            rate_code = data.split(":", 1)[1] if ":" in data else "SHAM"
            rate_code = rate_code.upper()

            if rate_code == "USD":
                await query.answer("💵 الدولار ثابت: 1$ = 1$ ولا يحتاج تعديل.", show_alert=True)
                return

            if rate_code not in ("SHAM", "SYRIATEL"):
                rate_code = "SHAM"

            context.user_data.clear()
            context.user_data["state"] = "setting_exchange_rate"
            context.user_data["exchange_rate_code"] = rate_code

            current_rate = get_exchange_rate(rate_code)
            current_text = f"1$ = {current_rate:,.0f} ل.س" if current_rate else "غير محدد"
            rate_name = exchange_rate_name(rate_code)

            await query.edit_message_text(
                f"💱 <b>تعديل سعر صرف {rate_name}</b>\n\n"
                f"السعر الحالي: <b>{current_text}</b>\n\n"
                f"أرسل سعر 1 دولار عند الدفع عبر <b>{rate_name}</b> بالليرة السورية.\n"
                "مثال: <code>11500</code>",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[red("❌ إلغاء", "admin_money")]]),
            )
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "manual_deposit_amount":
        try:
            local_amount = float(text.replace(",", "."))
            if local_amount <= 0:
                raise ValueError
        except ValueError:
            method_code = context.user_data.get("deposit_method_code")
            if method_code == "USD":
                await update.message.reply_text("❌ أرسل مبلغ صحيح بالدولار. مثال: 10")
            else:
                await update.message.reply_text("❌ أرسل مبلغ صحيح بالليرة السورية. مثال: 100000")
            return
        ...
        if not method:
            context.user_data.clear()
            await update.message.reply_text("❌ طريقة الدفع غير متاحة حالياً.")
            return
        ...
        if method_code == "USD":
            usd_amount = round(local_amount, 4)
            amount_line = f"💵 المبلغ المحوّل: <b>{local_amount:.2f}$</b>"
            rate_line = "💱 السعر: <b>1$ = 1$</b>"
        else:
            usd_amount = round(local_amount / rate, 4)
            amount_line = f"💴 المبلغ المحوّل: <b>{local_amount:,.0f} ل.س</b>"
            rate_line = f"💱 سعر الصرف: <b>1$ = {rate:,.0f} ل.س</b>"

        context.user_data["deposit_amount"] = usd_amount
        context.user_data["deposit_local_amount"] = local_amount
        context.user_data["deposit_exchange_rate"] = rate
        context.user_data["state"] = "manual_deposit_proof"
        ...
        await update.message.reply_text(
            ...
            f"{amount_line}\n"
            f"{rate_line}\n"
            f"💵 سيضاف للرصيد: <b>{usd_amount:.2f}$</b>\n\n"
            "📸 أرسل صورة إثبات الدفع هنا.",
            parse_mode="HTML",
        )
        return
    ...
    if state == "payment_address":
        method_code = context.user_data.get("payment_method_code")
        if not text:
            await update.message.reply_text("❌ أرسل عنوان تحويل صحيح.")
            return
        with db() as conn:
            method = conn.execute(
                "SELECT code FROM deposit_methods WHERE code = ?",
                (method_code,),
            ).fetchone()
            if not method:
                context.user_data.clear()
                ...
                return
            conn.execute(
                "UPDATE deposit_methods SET address = ?, updated_at = ? WHERE code = ?",
                (text, now_iso(), method_code),
            )
        context.user_data.clear()
        await update.message.reply_text(
            "✅ تم حفظ عنوان التحويل.",
            reply_markup=InlineKeyboardMarkup([[blue("💳 فتح طريقة الدفع", f"admin_payment_method:{method_code}")]]),
        )
        return

    if state == "payment_desc":
        method_code = context.user_data.get("payment_method_code")
        with db() as conn:
            method = conn.execute(
                "SELECT code FROM deposit_methods WHERE code = ?",
            ...
            if not method:
                ...
                return
            conn.execute(
                "UPDATE deposit_methods SET description = ?, updated_at = ? WHERE code = ?",
                (text, now_iso(), method_code),
            )
        context.user_data.clear()
        await update.message.reply_text(
            "✅ تم حفظ وصف طريقة الدفع.",
            reply_markup=InlineKeyboardMarkup([[blue("💳 فتح طريقة الدفع", f"admin_payment_method:{method_code}")]]),
        )
        return
    ...
    if state == "setting_exchange_rate":
        try:
            rate = float(text.replace(",", "."))
            if rate <= 1:
                raise ValueError
        except ValueError:
            await update.message.reply_text("❌ أرسل سعر صرف صحيح. مثال: 11500")
            return

        rate_code = context.user_data.get("exchange_rate_code", "SHAM").upper()
        if rate_code == "USD":
            context.user_data.clear()
            await update.message.reply_text(
                "💵 الدولار ثابت دائماً: 1$ = 1$.",
                reply_markup=InlineKeyboardMarkup([[blue("💵 الرصيد والأسعار", "admin_money")]]),
            )
            return
        ...
        set_setting(exchange_rate_setting_key(rate_code), rate)
        rate_name = exchange_rate_name(rate_code)
        context.user_data.clear()
        await update.message.reply_text(
            f"✅ تم تحديث سعر صرف {rate_name}.\n\n💱 <b>1$ = {rate:,.0f} ل.س</b>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[blue("💵 الرصيد والأسعار", "admin_money")]]),
        )
        return
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("state") != "manual_deposit_proof":
        return

    user = update.effective_user
    method_code = context.user_data.get("deposit_method_code")
    amount = context.user_data.get("deposit_amount")
    local_amount = context.user_data.get("deposit_local_amount", amount)
    exchange_rate = context.user_data.get("deposit_exchange_rate", 1)
    ...
    if not method or amount is None:
        context.user_data.clear()
        await update.message.reply_text("❌ انتهت جلسة الإيداع. ابدأ من جديد.")
        return
    ...
    with db() as conn:
        cur = conn.execute(
            ...
            (user_id, method_code, amount, local_amount, exchange_rate, proof_file_id, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?)
            """,
            (user.id, method_code, amount, local_amount, exchange_rate, photo.file_id, now_iso(), now_iso()),
        )
    ...
    transfer_text = (
        f"💵 المحوّل: <b>{local_amount:.2f}$</b>"
        if method_code == "USD"
        else f"💴 المحوّل: <b>{local_amount:,.0f} ل.س</b>"
    )

    await update.message.reply_text(
        f"✅ <b>تم إرسال طلب الإيداع #{deposit_id}</b>\n\n"
        f"💳 {html.escape(method['name'])}\n"
        f"{transfer_text}\n"
        f"💵 الرصيد بعد القبول: <b>{amount:.2f}$</b>\n"
    ...
    try:
        ...
        await context.bot.send_photo(
            ...
            photo=photo.file_id,
            caption=(
                f"📥 <b>إيداع جديد #{deposit_id}</b>\n\n"
                f"👤 {html.escape(user.first_name or '')}\n"
                f"🆔 <code>{user.id}</code>\n"
                f"💳 {html.escape(method['name'])}\n"
                f"{owner_transfer_text}\n"
                f"💵 يضاف: <b>{amount:.2f}$</b>"
            ),
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[
                blue("✅ قبول", f"deposit_approve:{deposit_id}"),
        ...
        logger.warning("Could not notify owner about deposit: %s", exc)
