...
def get_exchange_rate(rate_code="USD"):
    ...
    }.get(rate_code, "usd_to_syp")

    rate = safe_float(get_setting(setting_key, "0"), 0.0)
    # 1 or less means the exchange rate has not been configured yet.
...
async def show_deposit_method(query, context, method_code):
    ...
    await query.edit_message_text(
        ...
        "بعد التحويل اضغط متابعة، ثم أرسل المبلغ الذي حولته بالليرة السورية وصورة الإثبات.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
...
async def admin_money(query):
    ...
    await query.edit_message_text(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [blue("💵 تعديل سعر الدولار", "setting_exchange_rate:USD")],
            [blue("💳 سعر شام كاش", "setting_exchange_rate:SHAM")],
            [blue("📱 سعر سيريتل كاش", "setting_exchange_rate:SYRIATEL")],
...
async def admin_payment_methods(query):
    ...
    await query.edit_message_text(
        "💳 <b>طرق الدفع</b>\n\n"
        "من هون بتحدد عنوان التحويل والوصف لكل من شام كاش وسيريتل كاش، وبتفعل أو توقف كل طريقة.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(rows),
    )
...
async def admin_deposits(query, page=0):
    ...
    for dep in deposits:
        ...
        rows.append([
            blue(
                f"📥 #{dep['id']} | {name[:14]} | {(dep['local_amount'] or dep['amount']):,.0f} ل.س",
...
async def admin_deposit_show(query, context, deposit_id):
    ...
    caption = (
        ...
        f"💴 المحوّل: <b>{local_amount:,.0f} ل.س</b>\n"
        f"💱 الصرف: <b>1$ = {rate:,.0f} ل.س</b>\n"
...
async def admin_settings(query):
    await query.edit_message_text(
        ...
        "💳 طرق الدفع: <b>شام كاش • سيريتل كاش</b>",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(
            [
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "home":
        ...
        await query.edit_message_text(
            f"💳 <b>{html.escape(method['name'])}</b>\n\n"
            f"{html.escape(method['description'])}\n\n"
            f"💱 سعر الصرف: <b>1$ = {rate:,.0f} ل.س</b>\n\n"
            "أرسل المبلغ الذي حولته <b>بالليرة السورية</b>.\n"
        ...
        if data == "admin_stats":
            ...
            with db() as conn:
                ...
                if not dep:
                    await query.answer("طلب الإيداع غير موجود.", show_alert=True)
                    return
                if dep["exchange_rate"] is None or float(dep["exchange_rate"] or 0) <= 1:
                    await query.answer(
            ...
            rate_code = data.split(":", 1)[1] if ":" in data else "USD"
            rate_code = rate_code.upper()
            if rate_code not in ("USD", "SHAM", "SYRIATEL"):
                rate_code = "USD"
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "manual_deposit_amount":
        ...
        await update.message.reply_text(
            f"💳 <b>{html.escape(method['name'])}</b>\n\n"
            f"{html.escape(method['description'])}\n\n"
            f"💴 المبلغ المحوّل: <b>{local_amount:,.0f} ل.س</b>\n"
            f"💱 سعر الصرف: <b>1$ = {rate:,.0f} ل.س</b>\n"
    ...
    if state == "setting_exchange_rate":
        ...
        rate_code = context.user_data.get("exchange_rate_code", "USD").upper()
        if rate_code not in ("USD", "SHAM", "SYRIATEL"):
            rate_code = "USD"
