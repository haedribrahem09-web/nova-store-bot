...
async def show_deposit_method(query, context, method_code):
    ...
    await query.edit_message_text(
        ...
        "بعد التحويل اضغط متابعة، ثم أرسل مبلغ الإيداع وصورة الإثبات.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
...
async def admin_deposits(query, page=0):
    ...
    for dep in deposits:
        ...
        rows.append([
            blue(
                f"📥 #{dep['id']} | {name[:18]} | {dep['amount']:.2f}$",
        ...
        if data == "admin_stats":
            ...
            with db() as conn:
                ...
                if not dep or dep["status"] != "pending":
                    ...
                    return
                conn.execute(
                    "UPDATE deposits SET status = 'rejected', updated_at = ? WHERE id = ?",
                    (now_iso(), deposit_id),
                )
            try:
                await context.bot.send_message(
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "manual_deposit_amount":
        try:
            ...
            await update.message.reply_text("❌ أرسل مبلغ صحيح. مثال: 10")
            return
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        cur = conn.execute(
            ...
            (user_id, method_code, amount, proof_file_id, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, 'pending', ?, ?)
            """,
            (user.id, method_code, amount, photo.file_id, now_iso(), now_iso()),
        )
