...
def init_db():
    with db() as conn:
        ...
        conn.execute(
            """
            INSERT OR IGNORE INTO deposit_methods(code, name, description, active, updated_at)
            VALUES('SHAM', 'شام كاش', 'حوّل المبلغ إلى حساب شام كاش الموضح هنا، ثم أرسل صورة إثبات التحويل.', 1, ?)
            """,
            (now,),
        )
        conn.execute(
            """
            INSERT OR IGNORE INTO deposit_methods(code, name, description, active, updated_at)
            VALUES('SYRIATEL', 'سيريتل كاش', 'حوّل المبلغ إلى حساب سيريتل كاش الموضح هنا، ثم أرسل صورة إثبات التحويل.', 1, ?)
            """,
            (now,),
        )
...
async def show_deposit_method(query, context, method_code):
    ...
    await query.edit_message_text(
        ...
        "اضغط متابعة ثم أرسل مبلغ الإيداع.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
...
async def admin_payment_methods(query):
    ...
    await query.edit_message_text(
        "💳 <b>طرق الدفع</b>\n\n"
        "من هون بتقدر تعدل وصف شام كاش وسيريتل كاش وتفعل أو توقف كل طريقة.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(rows),
    )
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ADMIN
        ...
        if data == "admin_stats":
            ...
            await query.edit_message_text(
                "📝 أرسل الوصف الجديد لطريقة الدفع.\n\n"
                "ممكن تكتب رقم الحساب وتعليمات التحويل بالطريقة اللي بدك ياها.",
